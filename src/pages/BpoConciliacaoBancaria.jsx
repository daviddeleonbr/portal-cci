...
export default function BpoConciliacaoBancaria() {
  ...
  // redeSel: { tipo: 'webposto'|'autosystem', id } | null
  const [redeSel, setRedeSel] = useState(null);
  const redeId = redeSel?.tipo === 'webposto' ? redeSel.id : '';
  const redeAutosystemId = redeSel?.tipo === 'autosystem' ? redeSel.id : '';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Mantém TODOS os clientes (incluindo Autosystem) para que o lookup por
        // rede funcione nas duas integrações.
  ...
  // Contagens por id (Map) para o seletor
  ...
  const carregar = useCallback(async () => {
    if (redeSel?.tipo === 'autosystem') {
      setError('Conciliação Bancária para redes Autosystem ainda não está disponível. A integração com o servidor remoto está em desenvolvimento.');
      return;
    }
