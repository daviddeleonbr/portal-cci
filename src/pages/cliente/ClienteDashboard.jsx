...
import {
  CheckCircle2, Coins, FileSpreadsheet,
...
export default function ClienteDashboard() {
  ...
  // ─── Pendencias (sangrias / extratos) ───────────────────────
  const [pendencias, setPendencias] = useState({ sangrias: [], extratos: [] });
    ...
    (async () => {
      try {
        ...
        if (diasUteis.length === 0) {
          setPendencias({ sangrias: [], extratos: [] });
          return;
        }
  ...
  const totalPendencias = pendencias.sangrias.length + pendencias.extratos.length;
