...
import {
  ...
  ArrowLeft, Equal, FolderOpen, GripVertical, Star
} from 'lucide-react';
import {
...
export default function Parametrizacoes() {
  ...
  const [modalRedes, setModalRedes] = useState(null); // máscara cujas redes estão sendo editadas
  ...
  const definirPadrao = async (m) => {
    ...
    try {
      await dreService.definirMascaraPadrao(m.id);
  ...
  const duplicar = async (m) => {
    try {
      setSaving(true);
      await dreService.duplicarMascara(m.id);
    ...
    <div>
      ...
      {modalRedes && (
        <ModalRedesMascara mascara={modalRedes}
          listarRedes={dreService.listarRedesDaMascara}
          definirRedes={dreService.definirRedesDaMascara}
    ...
    <Modal open={open} onClose={onClose} title={data ? 'Editar Máscara' : 'Nova Máscara DRE'} size="sm"
      footer={(
        <div className="flex justify-end gap-3">
          ...
          <button type="submit" form="form-mascara-dre" disabled={saving || !form.nome.trim()}
      ...
      <form id="form-mascara-dre" onSubmit={(e) => { e.preventDefault(); onSave(form); }} className="space-y-4">
        <div>
